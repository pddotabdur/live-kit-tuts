...
from livekit.plugins import faseeh, openai, silero,deepgram
...
#
# Customer recites their last-4 ID digits; we compare against the stored value.
# One mismatch is forgiven (a single-digit STT mishear is common enough),
# second mismatch ends the call. The LLM converts Arabic-word numbers
# ("واحد اثنين ثلاثة أربعة") into a digit string before passing to the tool;
# we then sanitize and compare server-side.

ID_WRONG_DIGITS_GOODBYE = (
    "نعتذر على الإزعاج، يبدو أن البيانات ما تخصك، الله يعطيك العافية ومع السلامة."
)
ID_ASK_QUESTION = (
    "للتأكد من هويتك طال عمرك، ممكن تعطيني آخر ٤ أرقام من هويتك الوطنية؟"
)
ID_REASK_QUESTION = (
    "ممكن تعيد آخر ٤ أرقام من هويتك الوطنية مرة ثانية من فضلك؟"
)
ID_MISMATCH_REASK = (
    "الأرقام ما طابقت طال عمرك، ممكن تعيد آخر ٤ أرقام من هويتك الوطنية؟"
)

# Arabic-Indic → ASCII digit map for tool-arg sanitization.
_AR_INDIC_DIGITS = str.maketrans("٠١٢٣٤٥٦٧٨٩", "0123456789")


class IDVerificationAgent(BaseCallAgent):
    # One mismatch is forgiven (likely a single-digit STT mishear). Second
    # mismatch ends the call. Counts mismatches only — unclear replies just
    # re-ask without consuming an attempt.
    MAX_ATTEMPTS = 2

    def __init__(self, data: CallData, *, chat_ctx: ChatContext | None = None) -> None:
        super().__init__(
            instructions=(
                "You just asked the customer (in Arabic) to give you the last 4 digits "
                "of their national ID. Listen to their Arabic reply and call exactly one tool.\n"
                "\n"
                "- digits_provided(digits): customer spoke 4 digits. Pass them as a "
                "4-character string of ASCII digits 0-9. Convert Arabic-word numbers "
                "to digit characters (e.g. 'واحد اثنين ثلاثة أربعة' → '1234', "
                "'صفر تسعة خمسة سبعة' → '0957'). If they say it as one number "
                "('ألف ومئتين وأربعة وثلاثين' or '١٢٣٤'), still pass '1234'.\n"
                "- unclear: customer asked you to repeat, refused, gave fewer than 4 "
                "digits, or replied off-topic (عيدي / ما فهمت / ما أتذكر / وش تقولين). "
                "Use this when no clear 4-digit answer is given — never guess digits."
            ),
            chat_ctx=chat_ctx,
        )
        self.data = data
    ...
    async def on_enter(self):
        await self.session.say(ID_ASK_QUESTION, allow_interruptions=False)
    ...
    async def digits_provided(self, ctx: RunContext[CallData], digits: str):
        ...
        Args:
            digits: 4-character string of ASCII digits, e.g. '1234'.
        """
        clean = re.sub(r"\D", "", digits.translate(_AR_INDIC_DIGITS))
        if len(clean) != 4:
            # LLM didn't return 4 parseable digits — re-ask without burning
            # an attempt (this is a system issue, not a customer mismatch).
            await self.session.say(ID_REASK_QUESTION, allow_interruptions=False)
            return
        ...
        if self._attempts >= self.MAX_ATTEMPTS:
            ...
            await self.hangup()
            return
        await self.session.say(ID_MISMATCH_REASK, allow_interruptions=False)

    @function_tool()
    async def unclear(self, ctx: RunContext[CallData]):
        """User asked to repeat or didn't give a clear 4-digit reply."""
        await self.session.say(ID_REASK_QUESTION, allow_interruptions=False)
    ...
    def __init__(self, data: CallData, *, chat_ctx: ChatContext | None = None) -> None:
        super().__init__(
            instructions=(
                ...
                "\n\n"
                "Tools:\n"
                "- will_pay_full_now: agrees to pay full today "
                "(نعم / موافق / حول لي الرابط / أبشر / yes).\n"
                "- wants_partial: willing but only PART today / says full is too much "
                "(أقدر على جزء / نص المبلغ / مو كامل / not full).\n"
                "- cant_pay_today: cannot pay anything today, asks for later "
                "(ما أقدر اليوم / بعدين / بكرا / ما معي ولا ريال).\n"
                "- denies_debt: denies owing or claims paid (مو علي / سددته / مدفوع).\n"
                "- is_angry: insulting, frustrated, or accuses you of being a bot "
                "(أنتي AI / تستهبلين / why are you calling).\n"
                "- unclear_response: ambiguous, off-topic, or asks to repeat "
                "(عيدي / ما فهمت / يمكن / ماني عارف). "
                "Use whenever NOT confident — never guess.\n"
                "\n"
    ...
    def __init__(self, data: CallData, *, chat_ctx: ChatContext | None = None) -> None:
        super().__init__(
            instructions=(
                ...
                "\n"
                "Critical: a reply like 'I'll try' or 'maybe a little' WITHOUT a number "
                "is unclear, NOT partial_amount. Always require a concrete number.\n"
                "\n"
                "- partial_amount(amount): customer named a concrete amount. Pass digits only "
                "as a string (e.g. '50', '500', '٢٠٠' → '200', خمسين → '50', مية → '100'). "
                "Even small amounts count.\n"
                "- cant_pay_anything: cannot pay any amount today "
                "(ولا ريال / ما معي شي / مفلس).\n"
                "- unclear: ambiguous, off-topic, or asks to repeat. "
                "Use if no concrete amount and no clear refusal — never guess."
            ),
            chat_ctx=chat_ctx,
        )
        self.data = data
...
def prewarm(proc: JobProcess):
    # Load Silero VAD once per worker process so the first job doesn't pay
    # model-load latency on the critical path.
...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession[CallData](
        ...
        #stt=deepgram.STT(model="nova-3", language="ar-SA"),  # Hamsa: Gulf Arabic dialect-aware STT
        stt=hamsa_livekit.STT(language="ar"),
        llm=openai.LLM(model="gpt-4o-mini", temperature=0.3),
        # --- TTS (swap here when ready) ---
        #tts=hamsa_livekit.TTS(speaker="Hiba", dialect="ksa"),  # Hamsa KSA TTS
        tts=faseeh.TTS(
...
if __name__ == "__main__":
    cli.run_app(
        WorkerOptions(
            ...
            prewarm_fnc=prewarm,
            agent_name=os.getenv("AGENT_NAME", "outbound-caller-task"),
