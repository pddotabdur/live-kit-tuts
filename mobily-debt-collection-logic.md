...
flowchart TD
  ...
  S1 -->|Wrong party| WPQ[Ask: "Sorry do you know {title_with_name}?"]
...
- `"Hi, is this {title_with_name}?"`
- (If the customer greeted first) `"Hi yes, is this {title_with_name}?"`

`{title_with_name}` is gender-aware when input gender is available: male -> Mr./الأستاذ, female -> Ms./الأستاذة. If gender is unknown, use the name without a gendered title.
...
If the callee is **not** the named person but says they know them:

- Ask: `"Sorry do you know {title_with_name}?"`
...
- `"Thanks, {addressed_name}. This call may be recorded for quality."`

`{addressed_name}` uses `first_name` when provided, otherwise the first word of `name`; it is gender-aware: male -> Mr./أستاذ, female -> Ms./أستاذة. If gender is unknown, use the first name only.
...
If the customer says they do not know or recognize the debt/invoice (for example, “I can’t pay a debt I don’t know about”):

- Do **not** treat it as dispute yet.
- Explain 1–2 items from `services`: amount, subscription date when available, and **last 4 digits only** of the service number.
- Then ask: `"After the clarification, what’s the reason for the delay?"`
...
- **Unknown/unrecognized debt** → explain service details first, then capture reason
- **Immediate denial after details** → proceed to Stage 3 (but treat as dispute/denial path)
- **DNC / death** → end immediately (override)
...
- Restate payment options: SADAD app with national ID and biller code `005`, bank app, Mobily app, nearest Mobily branch, or ATM.
- Ask the customer to send the payment receipt after payment for confirmation.
- Add a short line that Nora will contact the callee on the agreed date to confirm.
- End with one confirmation question: “مضبوط؟”

Canonical template:

- `"Great just to confirm, this is the plan we agreed on: {plan_summary}. You can pay through SADAD using your national ID and biller code 005, through your bank app, through the Mobily app, at the nearest Mobily branch, or by ATM. After payment, send us the payment receipt for confirmation. I’ll call you on the agreed date to confirm. Is that correct?"`
