...
class IDVerificationAgent(BaseCallAgent):
    def __init__(self, data: CallData, *, chat_ctx: ChatContext | None = None) -> None:
        super().__init__(
            instructions=(
                ...
                "id_incorrect if they deny (لا / مو صح / غلط / مو هذي / لا مو صحيح), "
    ...
    async def id_incorrect(self, ctx: RunContext[CallData]):
        """Customer denied the digits — likely wrong account; end politely."""
...
class DebtContextAgent(BaseCallAgent):
    ...
    async def cant_pay_today(self, ctx: RunContext[CallData]):
        ...
        We do NOT jump straight to scheduling — a real collector probes for
        any partial amount first ("even a small amount today helps"). The
        partial-payment agent's 'cant_pay_anything' tool is the eventual
        fallback to ScheduleAgent if the customer truly cannot pay anything.
        """
        return PartialPaymentAgent(self.data, chat_ctx=self.chat_ctx)
